 ...
 * @notice Contract should be use only as proof of basic concept for smart contract usage in the field of agricutlural cooperatives
 */
...
contract SmartCOOP is Pausable, Ownable {
     ...
     * operational in Kovan testnet enviroment as well as in local ganache dev enviroment.
     */
    ...
    ///@dev Controling that function can be called only by EOA which is not already registred as SmartCOOP members (becomeCoopMember function)
    ...
    ///@dev Controling that fee payed when we call becomeCoopMember function is more then 1000 wei
    modifier minimumFee() {
    ...
    /// @return If execution was sucesfull function will return true
    ...
    /// @notice Allowing EOA which already pay SmartCOOP membershipe fee to deposit fruits to SmartCOOP warehouse
     ...
     * @dev Way of doing this is that who first deposit fruits will be first served ones buyers send his offer. Then function will continu to loop over
     * rest of producers till desired amount of fruits declared by bidder is satisfied. During this process function will erase precise amount of kilograms
     * associated to particular producer account. Then it will use chainlink oracle price feed to get USD/ETH price and to calculate how much SmartCOOP
     * should transfer to producer account. General principle is that 95% of total amount should be transfered to producer and 5% to SmartCOOP.
     * RASPBERRY_PRICE is hard coded because there is no out of the box chainlink date feed for raspberry price and alterantive solution includes using
     ...
     * @return true If execution was sucesfull function will return true
     */
    ...
    /// @param receiver Passing address to which COOPToken will be transfered.
    ...
    /// @notice This function is used to withdrow eth from SmartCOOP account
    ...
    /// @notice This function is used to provide overview of user SmartCOOP acount
    ...
    /// @return If executed sucesfully function will return 2 values: 1) total payed for all goods bidder bought; 2) Total amount of kg bidder bought from
    //  SmartCOOP
    ...
    /// @dev This function is used to unpause usage of becomeCoopMember, depositFruitsToCOOP and bid functions.
     ...
     * proper account for this purpose. NOTICE: For this to work please populate .env file in project root directory with you Kovan Infura endpoint
     * and private keys)
